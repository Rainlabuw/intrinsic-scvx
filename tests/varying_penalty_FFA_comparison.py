...
from src.models import IntrinsicPoweredDescentModel_FixedFinalAttitude, ClassicPoweredDescentModel_FixedFinalAttitude
import matplotlib.pyplot as plt
from src.parameters import PoweredDescentParameters
from src.algorithms import IntrinsicSCvx_FixedFinalAttitude, ClassicSCvx_FixedFinalAttitude
...
for penalty_coeff in penalty_coeff_hist:
    ...
    system = ClassicPoweredDescentModel_FixedFinalAttitude(params=params, x_init=x_init.copy())
    ...
    system = IntrinsicPoweredDescentModel_FixedFinalAttitude(params=params, x_init=x_init.copy())
    algo = IntrinsicSCvx_FixedFinalAttitude(params, system)
    x_opt, u_opt, traj_cost_hist, time, _ = algo.run(x.copy(), u.copy(), verbose=False)
...
plt.ylabel("Penalty")
plt.xlabel("penalty coeff")
plt.semilogx(penalty_coeff_hist, penalty_hist, label='iSCvx')
plt.semilogx(penalty_coeff_hist, cls_penalty_hist, label='SCvx')
plt.grid()
#plt.legend()

plt.tight_layout()
plt.savefig(f'media/big_comparison_FFA_{params.K}.pdf')
