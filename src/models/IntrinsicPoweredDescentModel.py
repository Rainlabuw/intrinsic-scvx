...
class IntrinsicPoweredDescentModel(PoweredDescentModel):
    ...
    # def A_matrix_2(self, x: np.ndarray, u: np.ndarray, x_next: np.ndarray) -> np.ndarray:
    ...
    #     A_coords = np.linalg.pinv(z_frame) @ A @ x_frame
    ...
    #     A_tilde = A@x_frame
    #     for i in range(12):
    #         A_tilde[:,i] = d_inv_retract(x_next, z, A_tilde[:,i])
    ...
    # def B_matrix_2(self, x: np.ndarray, u: np.ndarray, x_next: np.ndarray) -> np.ndarray:
    ...
    #     z = self.dynamics(x, u)
    #     z_frame = frame(z)
    #     B_coords = np.linalg.pinv(z_frame) @ B
    #     D = self.D_matrix(x_next, z)
    #     return D @ B_coords
